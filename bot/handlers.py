...
async def start_cmd(message: Message):
    welcome_text = """
🎬 **Video Downloader Bot — скачивание видео и фильмов по ссылке**

📌 **Что умеет:**
• Видео с популярных платформ  
• Фильмы с сайтов (адаптировано под Lordfilm и похожие)  
• Выбор качества и озвучки (если есть)  
• Показ прогресса и автоматический выбор лучшего качества

❗ **Важно:** бот не принимает прямые ссылки на потоки. Отправляйте ссылку на страницу видео/фильма.
...
async def status_cmd(message: Message, bot: Bot):
    ...
    status_lines.append(
        f"🎭 **Расширенный режим:** {'доступен' if video_service.playwright_available else 'не доступен'}"
    )
    db_pool = getattr(bot, "db_pool", None)
    if db_pool:
        try:
...
**Для фильмов:**
• Расширенный поиск источников на странице
• Автоматическое определение доступных потоков
• Отслеживание прогресса загрузки
• Поддержка прокси для обхода ограничений

**Поддерживаемые платформы (популярные примеры):**
• YouTube
• VK (ВКонтакте)
• RuTube
• Vimeo
...
**Фильмы с сайтов:**
• Сайты с плеерами (адаптировано под Lordfilm и похожие)

❗ **Важно:** прямые ссылки на потоки не принимаются — нужен URL страницы.
...
ℹ️ Полный список поддерживаемых сайтов меняется со временем.
"""
